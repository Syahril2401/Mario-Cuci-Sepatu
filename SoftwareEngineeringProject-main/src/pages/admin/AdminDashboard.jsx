...
const AdminDashboard = () => {
  ...
  const QUICK = [
    ...
    { label: 'Reports', icon: <FileText size={18} />, bg: '#f0fdf4', path: '/admin/report' },
  ];
  ...
  return (
    <div className="admin-root">
        ...
        <section className="adm-fade2" style={{ marginBottom: 18 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              <span className="adm-section-label" style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 0 }}>
            ...
            <button onClick={() => navigate('/admin/orders')}
              ...
              View All Order <ChevronRight size={13} />
            </button>
          </div>
      ...
      <section className="adm-fade3" style={{ marginBottom: 18 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          <button className="adm-quick-btn primary" style={{ flexDirection: 'row', justifyContent: 'center', width: '100%' }}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8 }}>
            {[
              { label: 'Manage\nOrders', icon: <ClipboardPen size={18} />, bg: '#e0f2fe', path: '/admin/orders', color: '#0369a1' },
              { label: 'View\nReports', icon: <FileText size={18} />, bg: '#dcfce7', path: '/admin/report', color: '#15803d' },
              { label: 'Promo\nManager', icon: <Tag size={18} />, bg: '#fef3c7', path: '/admin/promos', color: '#b45309' },
              { label: 'All\nServices', icon: <Wrench size={18} />, bg: '#ede9fe', path: '/admin/services', color: '#7c3aed' },
            ].map(q => (
              <button key={q.path} className="adm-quick-btn" onClick={() => navigate(q.path)} style={{ padding: '12px 4px', gap: 6 }}>
                <div className="adm-quick-icon" style={{ background: q.bg }}>{React.cloneElement(q.icon, { color: q.color })}</div>
                <span style={{ fontSize: 9.5, textAlign: 'center', lineHeight: 1.2, whiteSpace: 'pre-wrap' }}>{q.label}</span>
              </button>
            ))}
          </div>
        </div>
      ...
      <section>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
          <div className="adm-section-label" style={{ marginBottom: 0 }}>Order Selesai</div>
          <button onClick={() => navigate('/admin/orders', { state: { filter: 'FINISHED' } })}
            style={{ background: 'none', border: 'none', color: '#064058', fontSize: 11.5, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 2, cursor: 'pointer' }}>
